...
// PreInitialize loads the required_plugins defined in the config.
//
// This used to be part of `Initialize`, but was moved away since it needs
// to be run before `Initialize`, and we can stop if it fails.
func (cfg *PackerConfig) PreInitialize() hcl.Diagnostics {
	return cfg.detectPluginBinaries()
}
